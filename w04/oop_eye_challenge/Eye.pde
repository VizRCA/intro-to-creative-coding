...
class Eye {
  ...
  Eye(float x, float y, float diameter) {
    ...
    eyeColor = color(0, 190, 190); // default blue
  }
  ...
  void lookAt(float x, float y) {
    ...
    if (eyePointDistance < maxPupilRadius) {
      ...
      float dirX = x - locationX;
      float dirY = y - locationY;
      pupilLocationX = locationX + dirX * (maxPupilRadius/eyePointDistance);
      pupilLocationY = locationY + dirY * (maxPupilRadius/eyePointDistance);
    }
  }
